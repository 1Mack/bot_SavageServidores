...
exports.Up = async function (client, interaction, { id: discordUser } = discordUser, servidor, cargo, extra) {

    await interaction.deferReply()


    if (
        (discordUser == '323281577956081665' || ['fundador', 'diretor', 'gerente'].includes(cargo)) &&
    ...
    let fetchedUser
    try {
        fetchedUser = await interaction.guild.members.cache.get(discordUser);
    } catch (error) {
        return interaction.followUp({ embeds: [PlayerDiscordNotFound(interaction)], ephemeral: true })
    }


    const canal = client.guilds.cache.get('792575394271592458').channels.cache.find((channel) => channel.id === '792576052144373760');
    ...
    try {
        [rows] = await con.query(
            `select * from vip_sets where discord_id = "${discordUser}" AND server_id = (select id from vip_servers where server_name = "${servidor}")`
        );
    } catch (error) {
        return (
