const { TicketCompraPlanosOptions, TicketCompraPagamentosOptions, TicketCompraPlanosMenPerOptions } = require('../embed');
...
exports.compra_Handle = async function (m, user, msg) {
    ...
    let FirstAwnser = await m.awaitMessageComponent({ filter, componentType: 'SELECT_MENU', time: 45000 })
        .then(async (response) => {
    ...
    if (FirstAwnser !== 'outros') {
        ...
        FirstAwnser = await m.awaitMessageComponent({ filter, componentType: 'SELECT_MENU', time: 45000 })
            .then(async (response) => {
