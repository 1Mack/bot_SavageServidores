...
module.exports = {
    ...
    options: [{ name: 'bombs', type: 4, description: 'Total de Bombas', required: true, choices: [{ name: '1 (0.001x)', value: 1 }, { name: '3 (0.003x)', value: 3 }, { name: '5 (0.005x)', value: 5 }, { name: '10 (0.01x)', value: 10 }] },
    { name: 'creditos', type: 4, description: 'Total de créditos para apostar', required: true, choices: null }
    ],
    default_permission: false,
    cooldown: 15,
    permissions: [{ id: '748964438773727334', type: 1, permission: true }], //Perm ban
    async execute(client, interaction) {

        let bombs = interaction.options.getInteger('bombs'),
            creditos = interaction.options.getInteger('creditos'),
            creditosEarned = 0,
        ...
        switch (bombs) {
            case 1:
                creditosMultiplier = 0.001
                creditosMultiplierInitial = 0.0001
                break
            case 3:
                creditosMultiplier = 0.003
                creditosMultiplierInitial = 0.0003
                break
            case 5:
                creditosMultiplier = 0.005
                creditosMultiplierInitial = 0.0005
                break
            case 10:
                creditosMultiplier = 0.1
                creditosMultiplierInitial = 0.01
                break
        }
        let matriz = Array(5)
            .fill(null)
        ...
        while (!gameOver.bool) {
            ...
            await interaction.channel.awaitMessageComponent({ filter, componentType: 'BUTTON', time: 100000 })
                .then(async ({ customId }) => {
                    ...
                    embed.fields.find(m => m.name == 'Créditos ganhados até agora').value = `${creditosEarned}`
        ...
        if (!gameOver.won) {
            ...
            embed.fields.find(m => m.name == 'Créditos ganhados até agora').value = `${creditos}`
            embed.fields.find(m => m.name == 'Créditos ganhados até agora').name = `Créditos perdidos`
            embed.fields.find(m => m.name == 'Total de Créditos possuídos').value = `${result.credits}`
            ...
            embed.fields.find(m => m.name == 'Créditos ganhados até agora').value = `${creditosEarned}`
            embed.fields.find(m => m.name == 'Créditos ganhados até agora').name = `Créditos ganhados`
            embed.fields.find(m => m.name == 'Total de Créditos possuídos').value = `${result.credits}`
            embed.setDescription(`
            ▂▂▂▂▂▂▂▂▂▂▂▂▂▂▂▂▂▂▂▂
