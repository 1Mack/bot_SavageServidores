...
exports.CommandServidor = async function (interaction, command) {
  ...
  let response = await axios.post(`https://panel.mjsv.us/api/client/servers/${rows.identifier}/command`,
  ...
  if (!response) {
    interaction.followUp('***Aconteceu algum erro na hora de enviar o comando para o servidor!***').then(() => setTimeout(() => {
      interaction.deleteReply()
    }, 5000))
    return false
  }

  interaction.followUp(`***Servidor executando o comando: __${command}__***`)
