...
exports.AdminServidorRemover = async function (interaction, steamid, discord, tempo, serverID) {
  ...
  try {
    await con.query(`DELETE FROM Cargos WHERE playerid in (${steamid.map(m => `'${m}'`)}) AND server_id = ${serverID}`)
  } catch (error) {
    console.log(error)
    ...
    if (tempo)
      ...
      return interaction.followUp({ content: `Houve um erro ao remover o cargo, entre em contato com o 1MaaaaaacK`, ephemeral: true })
  }

  if (tempo) return true


  return interaction.followUp({ content: `Admins removidos com sucesso!` })
